...
class Stachowiak:
    """
    By using Stachowiak's lemmas, we construct the Hamiltonian cycles intended in his theorem from Hamiltonian paths in neighbor-swap graphs.
    The Hamiltonian paths are found by Verhoeff's algorithm. Then the are extended to Hamiltonian cycles by using Stachowiak's lemma 11 (which depends on the other lemmas).
    See the `main` function for more information on how to use this class. The lemma functions are also available for direct use.
    ...
    def _generate_all_di(self, chain_p: tuple[int, ...]) -> list[tuple[int, ...]]:
        """
    ...
    def _generate_all_di_prime(self, chain_p: tuple[int, ...]) -> list[tuple[int, ...]]:
        """
        Generate all possible `d_i` chains based on the given `chain_p`.
    ...
    def lemma2_cycle(
        self, chain_p: tuple[int, ...], case_2_1=True
    ) -> list[tuple[int, ...]]:
        """
        ...
        """
        if case_2_1:
            d_all = self._generate_all_di(chain_p)
        else:
            d_all = self._generate_all_di_prime(chain_p)
    ...
    def lemma2_extended_path(
        ...
        Returns:
            list[tuple[int, ...]]:
                ...
                - `d_i'=[l^i 1 l^{p-i} 0, l^i 1 l^{p-i-1} 0 l, \cdots, l^{i} 10 l^{p-i}, l^i 01 l^{p-i}, \cdots, l^i 0 l^{p-i} 1]`
        """
        cycle = self.lemma2_cycle(chain_p, case_2_1)
        if len(chain_p) % 2 == 0:
            if case_2_1:
    ...
    def _lemma8_helper(
        self, sig_occ: list[tuple[int, int]]
    ...
    def _lemma7_constructor(
        ...
        """
        return self._lemma8_helper([(0, 1), (1, 1), (2, sig[2]), (3, sig[3])])

    def lemma7(self, sig: list[int]) -> list[tuple[int, ...]]:
        """
        ...
        """
        q, suffix = self._lemma7_constructor(sig)
        cycle = [q[i] + suffix[i] for i in range(len(q))]
        return cycle

    def _cut_cycle_start_end(
        self, cyc: list[tuple[int, ...]], x: tuple[int, ...], y: tuple[int, ...]
    ) -> list[tuple[int, ...]]:
        """
    ...
    def _lemma8_g_i_sub_graphs(
        self, k_q: tuple[int, ...], l_p: tuple[int, ...]
    ) -> list[list[tuple[int, ...]]]:
        """
        ...
        for i in range(1, len(l_p) + 1):
            ...
            for j in range(len(l_p) - i + 1):
                ...
                # O <= j < (p-i)/2
                if j < (len(l_p) - i) / 2:
                    l7_q_set, l7_suffix = self._lemma8_helper(
                ...
                # j == (p-i)/2
                    ...
                    l7_subgraph = self.lemma7([1, 1, len(k_q), i])
                ...
                g_ij = self._cut_cycle_start_end(g_ij, x_ij, y_ij)
                g_i.extend(g_ij)
    ...
    def _lemma9_glue_a_edges(
        ...
        p = len(l_p)
        if len(k_s) > 0:
            g_result_start = self._cut_cycle_start_end(
                sub_cycles[0],
        ...
        for i in range(1, len(sub_cycles) - ((p + 1) % 2)):
            ...
            cyc = self._cut_cycle_start_end(sub_cycles[i], a_2i_1, a_2i_2)
        ...
        if p % 2 == 0:
            ...
            g_result_start.extend(
                self._cut_cycle_start_end(sub_cycles[-1], a_last_1, a_last_2)
            )
        g_result_start.extend(g_result_end[::-1])
        return g_result_start

    def lemma8(self, sig: list[int]) -> list[tuple[int, ...]]:
        """
        ...
        if sig[3] == 0:
            return g_0
        g_all = [g_0] + self._lemma8_g_i_sub_graphs(k_q, l_p)
        sub_cycles = []
        for i in range(0, len(g_all) - (len(g_all) % 2), 2):
            sub_cycles.append(g_all[i] + g_all[i + 1][::-1])
        if len(g_all) % 2 == 1:
            sub_cycles.append(g_all[-1])
        g_result_start = self._lemma9_glue_a_edges((), k_q, l_p, sub_cycles)
        return g_result_start

    def lemma9(self, sig: list[int]) -> list[tuple[int, ...]]:
        """
        ...
        if sig[3] == 0:
            ...
            for i in range(len(l_p) + 1):
                ...
                if ai_index > aj_index:
                    ...
                    ai_index = recursive_lists.index(ai)
                recursive_lists = (
                    recursive_lists[ai_index:] + recursive_lists[:ai_index]
                )
    ...
    def _lemma10_helper(
        self, K: list[tuple[int, ...]], p: int, new_color: int
    ) -> list[tuple[int, ...]]:
        """
        ...
        for i, gi in enumerate(G[1:], start=1):
            ...
            if adjacent(ai[0], aj[0]) and adjacent(ai[1], aj[1]):
                cycle, gi = self._lemma10_subcycle_cutter(cycle, gi, ai, aj)
            # if K_j and K_{j+1} don't differ in the first pair of elements, b_j and b_{j+1} are parallel
                ...
                cycle, gi = self._lemma10_subcycle_cutter(cycle, gi, bi, bj)
            cycle = [gi[0]] + cycle + gi[1:]
        return cycle

    def lemma10(self, sig: list[int]) -> list[tuple[int, ...]]:
        """
        ...
        cycle = self._lemma10_helper(K, sig[2], 2)
        return cycle

    def lemma11(self, sig: list[int]) -> list[tuple[int, ...]]:
        """
        ...
        if sig != sorted_sig:
            ...
            return transform(self.lemma11(sorted_sig), transformer)
        ...
        # if the sum of the first two odd numbers is less than 2, then there must be more 1's in the signature
            ...
            path = self.lemma2_extended_path(tuple([2] * sig[2]))
            next_color = 3
        else:
            raise ValueError(
        ...
        for ind, new_color in enumerate(sig[next_color:], start=next_color):
            cycle = self._lemma10_helper(path, new_color, ind)
            path = cycle
        return path


if __name__ == "__main__":
    stachowiak = Stachowiak()
    stachowiak.main()
