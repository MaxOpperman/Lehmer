...
def _lemma8_constructor(sig: List[int]) -> List[tuple]:
    """
    ...
    for i in range(1, sig[3]+1):
        ...
        for j in range(sig[3] - i + 1):
            ...
            # (p-i)/2 < j <= p-i
                ...
                for l7_i in range(len(l7_q_set)):
                    g_ij.append(l_p[:2*(sig[3]-i-j)] + l7_q_set[l7_i] + l_p[:i+2*j-sig[3]-1] + (0,) + l7_suffix[l7_i])
                g_ij = _lemma8_subgraph_cutter(
                    g_ij,
    ...
    # TODO: Fix this construction of the lemma
    for i in range(1, len(sub_cycles) + 1, 2):
        print(f"i-1 {i-1}", sub_cycles[i-1])
        cyc = _lemma8_subgraph_cutter(
            sub_cycles[i-1],
            (0,) + l_p[:sig[3]-i] + (1,) + l_p[:i-1] + (k_q[0], l_p[0]) + k_q[1:],
            (0,) + l_p[:sig[3]-i] + (1,) + l_p[:i] + k_q,
        )
        print(f"i {i}", sub_cycles[i])
        cyc2 = _lemma8_subgraph_cutter(
            sub_cycles[i],
            (0,) + l_p[:sig[3]-(i+1)] + (1,) + l_p[:i+1] + k_q,
            (0,) + l_p[:sig[3]-(i+1)] + (1,) + l_p[:i] + (k_q[0], l_p[0]) + k_q[1:],
        )
        g_result.extend(cyc)
        g_result.extend(cyc2)
        print(g_result, pathQ(g_result), cycleQ(g_result))
    # g_result = [x for xs in g_all for x in xs]
    return g_result
...
def lemma8(sig: List[int]) -> List[tuple]:
    """
     ...
     We assume sig has the form [1, 1, q, p]
    """
    q = _lemma8_constructor(sig)
    cycle = [q[i] for i in range(len(q))]
    # assert cycleQ(cycle)
    return cycle
