...
from core.type_variations.steinhaus_johnson_trotter_list import (
    SteinhausJohnsonTrotterList,
)
...
def _generate_all_di(chain_p: list[int]) -> list[list[int]]:
    """
...
def _generate_all_di_prime(chain_p: list[int]) -> list[list[int]]:
    """
    Generate all possible `d_i` chains based on the given `chain_p`.
    This function corresponds to the start of the proof of Lemma 2 (case 2.2 if the length of `chain_p` even).

    Args:
        chain_p (list[int]): The chain of p elements in Lemma 2.
...
def _lemma8_helper(
    ...
    k_q = [sig_occ[2][0]] * sig_occ[2][1]
    l_p = [sig_occ[3][0]] * sig_occ[3][1]
    if sig_occ[3][1] == 1:
        q, q2, cycle1, cycle2 = [], [], [], []
...
def lemma7(sig: tuple[int, ...]) -> list[list[int]]:
    ...
    Returns:
        list[list[int]]: A Hamiltonian cycle of the form `(0|1) (k^q|l^p)`
    """
...
def _cut_cycle_start_end(
    ...
    Args:
        cyc (list[list[int]]): Cycle in a subgraph.
        x (list[int]): Node that should be at the start of the path.
        y (list[int]): Node that should be at the end of the path.

    Returns:
        list[list[int]]: Subgraph with `x` as start and `y` as end.
...
def _lemma9_glue_a_edges(
    ...
    Args:
        k_r (list[int]): Chain of r elements "k"
        k_s (list[int]): Chain of s elements "k" (so  the same element as k_r but maybe a different length)
        l_p (list[int]): Chain of p elements "l" (different from k_r and k_s)
...
def lemma8(sig: tuple[int, ...]) -> list[list[int]]:
    ...
    Returns:
        list[list[int]]: A Hamiltonian cycle of the form `GE((0|1) k^q) | l^p)`
    """
    k_q = [2] * sig[2]
    l_p = [3] * sig[3]
    g_0, g_0_end = [], []
...
def _lemma10_subcycle_cutter(
    cycle: list[list[int]],
    gi: list[list[int]],
    edge_i: tuple[list[int], list[int]],
    edge_j: tuple[list[int], list[int]],
) -> tuple[list[list[int]], list[list[int]]]:
    """
    ...
    Args:
        cycle (list[list[int]]): The old cycle to cut for lemma 10.
        gi (list[list[int]]): The subgraph to cut for lemma 10, will be added later.
        edge_i (tuple[list[int], list[int]]):
...
def _lemma10_helper(K: list[list[int]], p: int, new_color: int) -> list[list[int]]:
    ...
    Returns:
        list[list[int]]: Hamiltonian cycle over `GE(Q | l^p)`
    """
        ...
        for item in g:
            ...
            if new_item.index(0) < new_item.index(1):
                ...
                for j, it in enumerate(new_item):
                    ...
                    if it == remove_color:
                        new_item[j] = new_color
            g_modified.append(new_item)  # Convert item back to a tuple
    ...
    for i, gi in enumerate(G[1:], start=1):
        ...
        ai = (l_p + K[2 * i], l_p[:-1] + [K[2 * i][0]] + l_p[-1:] + K[2 * i][1:])
        # b_i = (k_{2i} l^p, k_{2i,1} \dots k_{2i,q-1} l k_{2i,q} l^{p-1})
        bi = (
            K[2 * i] + l_p,
            K[2 * i][:-1] + l_p[-1:] + [K[2 * i][-1]] + l_p[:-1],
        )
