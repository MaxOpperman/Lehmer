...
class VerhoeffBinaryTheorem:
    ...
    \n
    Note:
        **Verhoeff's Binary Theorem** states the following:\n
        The neighbor-swap graph of binary (i.e., of arity 2) non-stutter permutations admits a Hamiltonian path. If both `k0` and `k1` are even, then there exists a Hamiltonian cycle.
        Moreover, we have:\n
        The path for the odd-even case `(k0, k1)` can run between `0^k0 1^k1` and `1^(k1-1) 0^k0 1`.
        Note that this path cannot avoid the edge `0^k0 1^k1` ~ `0^(k0-1) 10 1^(k1-1)`.\n
        The cycle for the even-even case `(k0, k1)` can include these two specific edges:\n
            - `0^(k0-2) 10 1^(k1-1) 0` ~ `0^(k0-1) 1^k1 0`
            - `1^(k1-2) 01 0^(k0-1) 1` ~ `1^(k1-1) 0^k0 1`
    References:
